...
class Invoice:
	...
	def __init__(self, in_no, name, amount):
		self.number = in_no
		self.customer = name
		self.amount = amount

	def add_payment(self, amt):
		self.total_payments += amt
		

	def is_fully_paid(self):
		if self.amount == self.total_payments:
			print('True')
		else:
			print('False')
...
class CustomerAccount:
	...
	def total_due(self):
		...
		for invoice in self.invoices:
			self.due +=invoice.amount_due()
		return self.due

	def unpaid_invoices(self):
            unpaid = []
            for invoice in self.invoices:
                if(invoice.amount != invoice.total_payments):
                    unpaid.append(invoice)
            return unpaid

	def apply_payment(self,pmt):
		for invoice in self.invoices:
			if(invoice.amount_due() >= pmt):
				invoice.add_payment(pmt)
				pmt = 0
			else:
				current_due = invoice.amount_due()
				invoice.add_payment(current_due)
				pmt = pmt - current_due
